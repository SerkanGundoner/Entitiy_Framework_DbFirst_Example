...
namespace EntityOrnek
{
    public partial class Form1 : Form
    {
        ...
        {
            SqlConnection baglanti = new SqlConnection(@"Data Source=DESKTOP-UFGU0VF;Initial Catalog=DbSinavOgrenci;Integrated Security=True");
        ...
        private void BtnLinqEntity_Click(object sender, EventArgs e)
        {
            ...
            {
                var topla = db.TBLNOTLAR.Sum(p => p.SINAV1);
                MessageBox.Show("Toplam SINAV1 Puanı: " + topla.ToString());
            }
            if (radioButton10.Checked == true)
            {
                var ortalama = db.TBLNOTLAR.Average(p => p.SINAV1);
                MessageBox.Show("SINAV1 Ortalaması:" + ortalama.ToString());
            }
            if (radioButton11.Checked == true)
            {
                var ortalamayuksek = db.TBLNOTLAR.Average(p => p.SINAV1);
                List<NOTLISTESI_Result> liste = db.NOTLISTESI().Where(p => p.SINAV1 > ortalamayuksek).ToList();
